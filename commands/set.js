...
module.exports = {
  data: {
    ...
    options: [
      ...
      },
      {
        type: "ROLE",
        name: "超激レア通知role",
        description: "超激レアの通知に使用するロールを選択",
      },
      {
        type: "ROLE",
        name: "tohru枠通知role",
  ...
  async execute(interaction) {
    ...
    const role2 = interaction.options.getRole("超激レア通知role");
    const role3 = interaction.options.getRole("tohru枠通知role");
    ...
    if(ch1) data[0].splice(0,0,ch1.id)
    if(ch2) data[0].splice(0,1,ch2.id)
    if(ch3) data[0].splice(0,2,ch3.id)
    if(role1) data[1].splice(0,0,role1.id)
    if(role2) data[1].splice(0,1,role2.id)
    if(role3) data[1].splice(0,2,role3.id)
    ...
    .addField(`= 超激レア通知用ch/role =`,`>>> ${data[0][1] != undefined ? "<#" + data[0][1] + ">" : "未設定"} / ${data[1][0] != undefined ? "<@&" + data[1][0] + ">" : "未設定"}`)
    .addField(`= tohru枠通知用ch/role =`,`>>> ${data[0][2] != undefined ? "<#" + data[0][] + ">" : "未設定"} / ${data[1][1] != undefined ? "<@&" + data[1][1] + ">" : "未設定"}`)
